...
export const DateFilterBar: React.FC<DateFilterBarProps> = ({
  ...
  // Collapsible toggle for filter presets and options ("botón desplegable")
  const [isExpanded, setIsExpanded] = useState<boolean>(() => {
    try {
      const saved = localStorage.getItem('gastoar_date_filter_expanded');
      if (saved !== null) return saved === 'true';
      // Default to false so it is collapsed under the dropdown button
      return false;
    } catch {
      return false;
    }
  ...
  const toggleExpanded = () => {
    setIsExpanded(prev => {
      const next = !prev;
      try {
        localStorage.setItem('gastoar_date_filter_expanded', String(next));
      } catch (e) {
        console.error(e);
      }
      return next;
  ...
  return (
    <div className={`bg-white rounded-2xl border transition-all p-3.5 sm:p-4 space-y-3 ${
      ...
      {/* Top row: Status, Active Period, Dropdown Button, Pin Button, Count & Actions */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
        <div className="flex items-center gap-2 flex-wrap">
          {/* Active filter pill / clickable trigger */}
          <button
            type="button"
            id="date-filter-active-pill"
            onClick={toggleExpanded}
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl font-bold text-xs border transition-all cursor-pointer select-none active:scale-95 ${
              isRangePinned 
                ? 'bg-indigo-600 text-white border-indigo-600 shadow-xs' 
                : 'bg-indigo-50 border-indigo-100 text-indigo-700 hover:bg-indigo-100'
            }`}
            title={isExpanded ? "Ocultar opciones de fecha" : "Desplegar opciones de fecha"}
          >
            {isRangePinned ? (
          ...
          {/* Botón Desplegable para ver/ocultar las opciones de fecha */}
          <button
            type="button"
            id="btn-toggle-date-filter-options"
            onClick={toggleExpanded}
          ...
          >
            ...
            <span>{isExpanded ? 'Ocultar opciones' : 'Opciones de fecha'}</span>
          ...
          {isFiltered && !isRangePinned && (
            <button
              ...
              className="inline-flex items-center gap-1 text-[11px] font-medium text-slate-500 hover:text-rose-600 bg-slate-100 hover:bg-rose-50 px-2.5 py-1.5 rounded-xl transition-colors cursor-pointer"
              title="Quitar filtro de fecha (Ver Todo)"
            >
              <X className="w-3 h-3" />
              <span>Ver Todo</span>
            </button>
      ...
      {/* Preset Pills, Month Dropdown & Pin Checkbox Card (Desplegable) */}
      {isExpanded && (
        <div className="pt-2 border-t border-slate-100 space-y-3 animate-in fade-in slide-in-from-top-1 duration-200">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              {presets.map((preset) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 whitespace-nowrap active:scale-95 cursor-pointer ${
                      isActive
              ...
              <div className="relative">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 border active:scale-95 cursor-pointer ${
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 flex flex-col sm:flex-row sm:items-center gap-3 animate-in fade-in duration-200">
              ...
              {(filters.startDate || filters.endDate) && (
                <button
                  ...
                  className="text-xs text-rose-600 hover:text-rose-700 font-semibold px-2 py-1 bg-rose-50 rounded-lg self-end sm:self-center cursor-pointer"
                >
                  Borrar Rango
                </button>
              )}
