import { CategoryMap, TransactionConfidence } from './types';
...
export function parseVoiceExpenseLocally(
  ...
  const metaKeywords = [
    'al fondo para', 'al fondo de', 'al fondo', 'fondo para', 'fondo de',
    'a la meta', 'para la meta', 'meta de', 'meta para',
    'ahorro para', 'para las vacaciones', 'para el viaje a', 'para mendoza', 'fondo mendoza'
  ];
  ...
  if (isMeta) {
    ...
    const matchFondo = lower.match(/(?:fondo para|fondo de|meta de|meta para|para el viaje a|al fondo para|para)\s+([a-záéíóúñ\s]+)/i);
    ...
    if (!metaNombre && lower.includes('mendoza')) metaNombre = 'Mendoza';
  } else if (
  ...
  const categoryConfidence = (!isGenericConcept || tipoOperacion !== 'gasto') ? 0.95 : 0.50;
  // Payment method confidence is strictly low if user didn't mention it in voice
  const paymentMethodConfidence = hasExplicitPaymentMethod ? 0.95 : 0.35;
  const installmentsConfidence = (metodoPago !== 'Crédito' || hasExplicitInstallments) ? 0.98 : 0.60;
